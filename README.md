...
Use [`@fastify/valkey-glide`](https://github.com/fastify/fastify-valkey-glide) to share a GLIDE client with the rate limiter:
...
Register `@fastify/valkey-glide` first so its client is available. Install `@valkey/valkey-glide` at the application root, as above, so both plugins use the same GLIDE installation. The `valkey` option accepts both standalone and cluster clients, including a caller-owned `GlideClient` or `GlideClusterClient`.
...
- `timeWindow:` the duration of the time window. It can be expressed in milliseconds, as a string (in the [`ms`](https://github.com/vercel/ms) format), or as an async function with the signature `async (request, key) => {}` where `request` is the Fastify request object and `key` is the value generated by the `keyGenerator`. The function **must** return a number.
...
- `redis`: by default, this plugin uses an in-memory store, but if an application runs on multiple servers, an external store will be needed. This plugin requires the use of [`ioredis`](https://github.com/redis/ioredis).<br> **Note:** the [default settings](https://github.com/redis/ioredis/blob/v4.16.0/API.md#new-redisport-host-options) of an ioredis instance are not optimal for rate limiting. We recommend customizing the `connectTimeout` and `maxRetriesPerRequest` parameters as shown in the [`example`](https://github.com/fastify/fastify-rate-limit/blob/main/example/example.js).
- `valkey`: use a `GlideClient` or `GlideClusterClient` from [`@valkey/valkey-glide`](https://github.com/valkey-io/valkey-glide) instead of `redis`. In Fastify applications, [`@fastify/valkey-glide`](https://github.com/fastify/fastify-valkey-glide) can manage this client.
...
- `keyGenerator`: a sync or async function to generate a unique identifier for each incoming request. Defaults to `(request) => normalizeIP(request.ip, ipv6Subnet)`, the IP is resolved by fastify using `request.connection.remoteAddress` or `request.headers['x-forwarded-for']` if [trustProxy](https://fastify.dev/docs/latest/Reference/Server/#trustproxy) option is enabled. The default normalizes IP strings, maps IPv4-mapped IPv6 addresses to IPv4, and masks IPv6 addresses to the configured `ipv6Subnet`. Use it if you want to override this behavior
- `ipv6Subnet`: the IPv6 prefix length used by the default `keyGenerator`. Defaults to `64`. Set it to another prefix length when your clients commonly receive a different IPv6 allocation size. The `normalizeIP(ip, ipv6Subnet)` helper is also exported for custom key generators.
...
`allowList`, `keyGenerator`, `ipv6Subnet`, and `ban`.
...
- **`ttl` reflects the store's current window.** With the Redis and Valkey stores, `ttl` is the raw server `PTTL` (the same value `incr` reports), so it can exceed the configured `timeWindow` when `continueExceeding`/`exponentialBackoff` has extended it.
- **Custom stores must implement `read`.** The flag relies on a non-mutating `read(ip, cb, timeWindow, max)` method, which mirrors the `incr` signature. The built-in local, Redis, and Valkey stores provide it; a custom store that does not will throw a clear error when called with `{ increment: false }`.
...
### Contribute
To run tests locally with Redis:
```
npm run redis
```

To include the Valkey integration test:
```
npm run valkey
VALKEY_HOST=127.0.0.1 npm test
```
